...
dat$mla_1[is.na(dat$mla_1)] = 0       # add a 0 in place of NA for removal
dat = dat[dat$mla_1 != 0,]            # remove 0 in mla_1
unique(dat$mla_1)                     # 0's and NA's absent 
dat$mla_1 = log(dat$mla_1)
...
off_t1 = 0.0975
ppi = 600
png("writing/figures/lmb_mla_1_log_small.png", h = 3.5 * ppi, w = 4 * ppi, res = ppi)

# small lakes
par(mfrow = c(1,1), mar = c(0.5,1,0.5,1), oma = c(1.5,3,0.5,0), cex.axis = 1, cex.lab = 1.2, mgp = c(1,0.75,0))
plot(1,1, type = "n", xlim = c(2017,2019) + range(c(off_c1, off_c2, off_t1, off_t2)), ylim = c(4.9,5.7),
     xaxt = "n", yaxt = "n", las = 1, ylab = "")
...
legend(usr[1], usr[4] - 0.05, legend = c("Control", "Treatment", "Untreated", "Treated"), ncol = 2,
...
axis(side = 2, at = c(5, 5.5), labels = c(5, 5.5), las = 1)
mtext(side = 2, outer = T, "ln Age-1 Mean TL (mm)", line = 1.75, cex = 1.2) 
dev.off()
