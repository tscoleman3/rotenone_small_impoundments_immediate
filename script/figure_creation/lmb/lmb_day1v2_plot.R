...
xc2 = x + 0.06
xt2 = x - 0.06
png("lmb_day1_day2.png", h = 7 * ppi, w = 4 * ppi, res = ppi)
par(mfrow = c(2,1), mar = c(0.5,1,0.5,1), oma = c(2.5,3,0.5,0), cex.axis = 1, cex.lab = 1.2, mgp = c(1,0.75,0))
plot(x = 1, y = 1, type = "n", xlab = "", ylab = "", xlim = c(0.75,2.25), ylim = range(seq(0,1120)),las = 1, xaxt = "n")   
...
plot(x = 1, y = 1, type = "n", xlab = "", ylab = "", xlim = c(0.75,2.25), ylim = range(big_lwr, big_upr) * c(1,1.15),las = 1, xaxt = "n")    
usr = par("usr"); xdiff = usr[2] - usr[1]; ydiff = usr[4] - usr[3]
