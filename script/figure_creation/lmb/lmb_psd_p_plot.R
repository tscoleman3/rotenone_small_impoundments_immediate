...
off_t1 = off_t2/2
ppi = 600
png("lmb-psd-p.png", h = 7 * ppi, w = 4 * ppi, res = ppi)
...
# big lakes
plot_dat_big
plot(1,1, type = "n", xlim = c(2017,2019) + range(c(off_c1, off_c2, off_t1, off_t2)), ylim = range(plot_dat_big[,c("lwr", "upr")]) * c(1, 1.05),
     xaxt = "n", las = 1, ylab = "")
usr = par("usr"); xdiff = usr[2] - usr[1]; ydiff = usr[4] - usr[3]
...
mtext(side = 2, outer = T, "LMB Preferred Proportional Size Distribution (PSD-P)", line = 1.75, cex = 1.2) 
dev.off()
