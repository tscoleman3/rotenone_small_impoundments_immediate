...
dat$surv[is.na(dat$surv)] = 0        # add a 0 in place of NA for removal
dat = dat[dat$surv != 0, ]           # remove 0 in surv
...
plot_dat_sml = datsub %>% filter(size_class == "small") %>%
  ...
  summarize(
    mn = mean(surv, na.rm = T),
    lwr = quantile(surv, 0.025, na.rm = T),
    upr = quantile(surv, 0.975, na.rm = T)
    ) %>% ungroup
...
off_t1 = off_t2/2
ppi = 600
png("lmb_surv.png", h = 7 * ppi, w = 4 * ppi, res = ppi)

# small lakes
par(mfrow = c(2,1), mar = c(0.5,1,0.5,1), oma = c(1.5,3,0.5,0), 
    cex.axis = 1, cex.lab = 1.2, mgp = c(1,0.75,0))
plot(1,1, type = "n", xlim = c(2018,2019) + range(c(off_c1, off_c2, off_t1, off_t2)), 
     ylim = range(plot_dat_sml[,c("lwr", "upr")]) * c(1, 1.15),
     xaxt = "n", las = 1, ylab = "")
...
points(mn ~ I(year + off_t1), data = plot_dat_sml[ind,], pch = 21, 
       bg = c("white", "black", "black"), cex = 1.2)
...
points(mn ~ I(year + off_t2), data = plot_dat_sml[ind,], pch = 21,
       bg = c("white", "black"), cex = 1.2)
...
plot_dat_big
plot(1,1, type = "n", xlim = c(2018,2019) + range(c(off_c1, off_c2, off_t1, off_t2)), 
     ylim = range(plot_dat_big[,c("lwr", "upr")]) * c(1, 1.15),
     xaxt = "n", las = 1, ylab = "")
usr = par("usr"); xdiff = usr[2] - usr[1]; ydiff = usr[4] - usr[3]
text(x = usr[2] - xdiff * 0, y = usr[4] - ydiff * 0.05, pos = 2, "Small", font = 4)
...
points(mn ~ I(year + off_t1), data = plot_dat_big[ind,], pch = 21, 
       bg = c("white", "black", "black"), cex = 1.2)
...
mtext(side = 2, outer = T, "LMB Survival", line = 1.75, cex = 1.2) 
dev.off()
