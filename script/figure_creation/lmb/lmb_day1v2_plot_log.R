...
big_mean = with(subset(df, size == "big"), 
                tapply(count, list(event, period, type), mean))
big_lwr = with(subset(df, size == "big"), 
               tapply(count, list(event, period, type), function(x) quantile(x, 0.025)))
big_upr = with(subset(df, size == "big"), 
               tapply(count, list(event, period, type), function(x) quantile(x, 0.975)))
...
xc2 = x + 0.045
xt2 = x - 0.045
png("writing/figures/manuscript/lmb_day1_day2_log.png", h = 7 * ppi, w = 4 * ppi, res = ppi)
...
plot(x = 1, y = 1, type = "n", xlab = "", ylab = "", xlim = c(0.75,2.25), 
     ylim = range(sml_lwr, sml_upr) * c(1,1.15),las = 1, xaxt = "n")   
...
legend("top", legend = c("Control", "Treatment", "App. 1", "App. 2"), 
...
plot(x = 1, y = 1, type = "n", xlab = "", ylab = "", xlim = c(0.75,2.25), 
     ylim = range(seq(0,8)) * c(1,1.15),las = 1, xaxt = "n")    
usr = par("usr"); xdiff = usr[2] - usr[1]; ydiff = usr[4] - usr[3]
...
# text(x = x, y = usr[3] - ydiff * 0.09, 
#      labels = c("Day 1 & 21\nPre-Treatment", "Day 2 & 22\nPost-Treatment"), xpd = T, cex = 1.1)
