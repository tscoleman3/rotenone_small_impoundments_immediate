...
off_t1 = 0.0975
ppi = 600
png("lmb_psd.png", h = 7 * ppi, w = 4 * ppi, res = ppi)

# small lakes
par(mfrow = c(2,1), mar = c(0.5,1,0.5,1), oma = c(1.5,3,0.5,0), cex.axis = 1, cex.lab = 1.2, mgp = c(1,0.75,0))
plot(1,1, type = "n", xlim = c(2017,2019) + range(c(off_c1, off_c2, off_t1, off_t2)), ylim = c(0,5),
     xaxt = "n", las = 1, ylab = "")
...
# big lakes
plot_dat_big
plot(1,1, type = "n", xlim = c(2017,2019) + range(c(off_c1, off_c2, off_t1, off_t2)), ylim = c(2,4),
     xaxt = "n", las = 1, ylab = "")
usr = par("usr"); xdiff = usr[2] - usr[1]; ydiff = usr[4] - usr[3]
...
mtext(side = 2, outer = T, "ln LMB Proportional Size Distribution (PSD)", line = 1.75, cex = 1.2) 
dev.off()
