...
dat = read.csv("data/blue_seine.csv", stringsAsFactors = F)
...
sml_lwr = with(subset(df, size == "small"), tapply(count, list(event, period, type), function(x) quantile(x, 0.025)))
sml_upr = with(subset(df, size == "small"), tapply(count, list(event, period, type), function(x) quantile(x, 0.975)))
big_mean = with(subset(df, size == "big"), tapply(count, list(event, period, type), mean))
big_lwr = with(subset(df, size == "big"), tapply(count, list(event, period, type), function(x) quantile(x, 0.025)))
big_upr = with(subset(df, size == "big"), tapply(count, list(event, period, type), function(x) quantile(x, 0.975)))
...
xc2 = x + 0.045
xt2 = x - 0.045
png("writing/figures/manuscript/age0/blue_day1_day2_log.png", h = 7 * ppi, w = 4 * ppi, res = ppi)
par(mfrow = c(2,1), mar = c(0.5,1,0.5,1), oma = c(2.5,3,0.5,0), cex.axis = 1, cex.lab = 1.2, mgp = c(1,0.75,0))
plot(x = 1, y = 1, type = "n", xlab = "", ylab = "", xlim = c(0.75,2.25), ylim = range(seq(1,8,1)) * c(1,1.15),las = 1, xaxt = "n")     # change ylim for log()
...
legend("top", legend = c("Control", "Treatment", "App. 1", "App. 2"), ncol = 2, pch = c(NA, NA, 15, 15), seg.len = c(2.75,2.75,1,1),
       col = c(rep("black", 3), "grey75"), bty = "n", pt.cex = 1.5, lty = c(2,1,NA,NA), lwd = c(3,3,NA,NA), x.intersp = c(1,1,0.25,0.25))
...
points(big_mean[2,,"control"] ~ xc2, type = "l", pch = 16, lwd = 3, cex = 1.2, lty = 2, col = "grey75")
points(big_mean[2,,"treat"] ~ xt2, type = "l", pch = 16, lwd = 3, cex = 1.2, col = "grey75")
...
arrows(xc2, big_lwr[2,,"control"], xc2, big_upr[2,,"control"], length = 0, lwd = 3, lty = 2, col = "grey75")
arrows(xt2, big_lwr[2,,"treat"], xt2, big_upr[2,,"treat"], length = 0, lwd = 3, col = "grey75")
axis(side = 1, at = x, labels = rep("", 2))
text(x = usr[2] - xdiff * 0, y = usr[4] - ydiff * 0.05, pos = 2, "Large", font = 4)
mtext(side = 2, outer = T, "ln Total Bluegill Catch Per Impoundment", line = 1, cex = 1.2)                                                   
