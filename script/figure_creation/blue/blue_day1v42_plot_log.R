...
sml_lwr = with(subset(df, size == "small"), tapply(count, list(period, type), function(x) quantile(x, 0.1)))
sml_upr = with(subset(df, size == "small"), tapply(count, list(period, type), function(x) quantile(x, 0.9)))
big_mean = with(subset(df, size == "big"), tapply(count, list(period, type), mean))
big_lwr = with(subset(df, size == "big"), tapply(count, list(period, type), function(x) quantile(x, 0.1)))
big_upr = with(subset(df, size == "big"), tapply(count, list(period, type), function(x) quantile(x, 0.9)))

ppi = 600
x = c(0.85,2.15)
xc = x + 0.015
xt = x - 0.015
png("blue_day1_42_log.png", h = 7 * ppi, w = 4 * ppi, res = ppi)
par(mfrow = c(2,1), mar = c(0.5,1,0.5,1), oma = c(1.5,3,0.5,0), cex.axis = 1, cex.lab = 1.2, mgp = c(1,0.75,0))
plot(x = 1, y = 1, type = "n", xlab = "", ylab = "", xlim = c(0.75,2.25), ylim = range(seq(5,9,1)), las = 1, xaxt = "n")    
...
mtext(side = 2, outer = T, "ln Total Bluegill Catch Per Impoundment", line = 1.75, cex = 1.2)                                                       
